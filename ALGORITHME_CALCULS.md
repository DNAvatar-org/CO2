...
L'algorithme principal est dans `simulateRadiativeTransfer()` qui gère :
1. **Boucle extérieure** : Changements d'époque, événements (météorites, ticTime), recalculs
2. **Boucle intérieure** : Itérations de convergence (dichotomie/exponentielle) pour trouver T0
