...
- **Actuel** : `physics.js` définit des constantes (CO2_SIGMA_*, H2O_SIGMA_*, CH4_SIGMA_*) ; `calculations.js` expose `crossSectionCO2(λ)`, `crossSectionH2O(λ)`, `crossSectionCH4(λ)` — formules gaussiennes en λ, **sans** dépendance T/P dans la forme (un simple `pressureBroadening = √(P/P_ref)` est appliqué côté κ).
...
4. **Fonctions de scaling** (dans `physics.js` ou un module dédié `hitran_utils.js`) :
...
6. **Intégration dans `calculations.js`** :
