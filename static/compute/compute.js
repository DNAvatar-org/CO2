...
function getAnimState() {
    ...
    const animToggle = typeof document !== 'undefined' 
        ? (document.getElementById('plot-anim-toggle') || document.getElementById('anim-toggle'))
        : null;
    return animToggle ? animToggle.checked : false;
}
...
function getMasses(dateConfig, epoch) {
    ...
    // Créer l'objet avec logos (incluant la masse totale)
    const masses = {
        '🏭🐳': co2_kg,
        '⛽🐳': ch4_kg,
        '💧🐳': h2o_kg,
    ...
    const massesStr = `{'🏭🐳': ${co2_kg.toExponential(2)}, '⛽🐳': ${ch4_kg.toExponential(2)}, '💧🐳': ${h2o_kg.toExponential(2)}, '🌫🐳': ${o2_kg.toExponential(2)}, '🐳': ${total_atmosphere_mass_kg.toFixed(0)}}`;
...
function getEpochDateConfig() {
    ...
    // Retourner un objet avec des logos comme clés (sans les masses, elles sont dans getMasses())
    const result = {
        '🎬': animEnabled ?? false,           // Animation (booléen)
        '🏮': prev_T0_val,                     // old_T0 (température précédente, peut être null)
        '🌡️': t0_config,                       // t0_config (température config, peut être null)
        '☄️': meteoriteCount ?? 0,              // Nombre de météorites
        '🌡️☄️': deltaMeteorite ?? 0,           // Delta température météorite
        '☄️💧🐳': water_added_kg,               // Masse d'eau ajoutée par météorite (kg)
        '🕓': ticTime ?? 0,                     // TicTime
    ...
    console.log(`dateConfig={'🎬':${animEnabled}, '🏮':${old_T0_str}, '🌡️':${t0_config_str}, '☄️':${meteoriteCount}, '🌡️☄️':${deltaMeteorite_str}, '☄️💧🐳':${water_added_str}, '🕓':${ticTime}, '🌡️🕓':${deltaTicTime_str}}`);
